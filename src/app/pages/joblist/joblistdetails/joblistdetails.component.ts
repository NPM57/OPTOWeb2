...
@Component({
	selector: 'job-list-details',
	templateUrl: './joblistdetails.html',
	styleUrls: ['./joblistdetails.scss']
})

export class JobListDetails {
	...
	ngOnInit(){
		...
		this.service.getWorkCenterDetail(20).subscribe(res => {
		...
		.subscribe(() => { 
			this.service.getWorkCenterDetail(20)
			.subscribe(res => {
				this.tableData = res.json()["items"];
			});
		});
	...
	TopClockOn():void{
		this.clockonservice.getAllInformation(this.id).subscribe(res=>{
			this.clock_on = res.json()["clocked_on"];
			if(this.clock_on!=0){
				alert("This employee already clocked on another job")
			}else{
				this.clockonservice.clockOn(this.id).subscribe(res=>{
					alert("Clock on success");
				});
			}
		})
	}
